...
class OLS:
    ...
    def __init__(self, y, x, y_varnm='y', x_varnm='', only_b=False, hasconstant=False):
        """
    ...
    def cond_number(self):
        norm_x = np.ones_like(self.x)
        for i in range(int(self.df_model)):
            norm_x[:, i] = self.x[:, i] / np.linalg.norm(self.x[:, i])
    ...
    def ll(self):
        ...
        ll = -(self.nobs*1/2)*(1+log(2*pi)) - (self.nobs/2)*log(dot(self.e.T,self.e)/self.nobs)
    ...
    def summary(self):
        ...
        for i in range(len(self.x_varnm)):
            print '''% -5s          % -5.6f     % -5.6f     % -5.6f     % -5.6f''' % tuple([self.x_varnm[i],self.b[i],self.se[i],self.t[i],self.pvalues[i]])
        print '=============================================================================='
